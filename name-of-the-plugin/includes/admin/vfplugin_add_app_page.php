...
if (!class_exists('vfplugin_App_Init'))
{

  	/**
  	*
  	*/
  	class vfplugin_App_Init
    {

        function vfplugin_sso_add_app()
        {
            global $wpdb;

            $table_name = $wpdb->prefix.'oauth_clients';

            if ( isset($_POST['submit_vfplugin_app'] ) )
            {

                if ( ! isset( $_POST['vfplugin_app_nonce'] ) || ! wp_verify_nonce( $_POST['vfplugin_app_nonce'], 'add_vfplugin_app_nonce' ) ) {

                    wp_die( __( 'Cheatin&#8217; huh?', 'vfplugin_app' ) );
                ...
                else
                {
                    if ( empty($_POST['app_uname']) && empty($_POST['app_uri']) )
                    {
                        ...
                        <div class="notice notice-error is-dismissible">
                            <p><?php _e( 'Fill All Fields.', 'vfplugin_app' ); ?></p>
                        </div>
                        <?php
                    }
                    else
                    {
                        ...
                        $consumer_key = VFPLUGIN_TokenGenerator::VFPLUGIN_GENERATETOKEN();
                        $secret_key = VFPLUGIN_TokenGenerator::VFPLUGIN_GENERATETOKEN();
            ...
            if ( isset($_GET['msg'] ) && $_GET['msg']==1 ) {
                ...
                <div class="notice notice-success is-dismissible">
                    <p><?php _e( 'App Added Successfully.', 'vfplugin_app' ); ?></p>
                </div>
                <?php
            }
            ...
            <form action="" method="post">
                ...
                <?php wp_nonce_field( 'add_vfplugin_app_nonce', 'vfplugin_app_nonce' ); ?>

				        <p><input type="submit" name="submit_vfplugin_app" class="button button-primary" value="Save" /></p>
